...
HashTable::~HashTable()
{
    ...
    // and free up the link in the node if any exists
    for (const auto node : nodes)
    {
        Node* prev = node.next; // Variable for holding reference to the node to delete
        Node* curr = prev;      // Variable for traversal
        while (curr != nullptr)
        {
            // Take the current node and delete it
            prev = curr;
            delete prev;

            // Move the node to the next position
            curr = curr->next;
        }
    }
...
void HashTable::Insert(Bid bid)
{
    ...
    const auto key = hash(static_cast<unsigned int>(strToDouble(bid.bidId, ' ')));
    // retrieve node using key
    auto found = &nodes[key];
    // if no entry found for the key
    if (found->key == UINT_MAX)
    {
    ...
    else
    {
        ...
        // If the next node after the initial node is null,
        // Then, we insert it into the node's next position
        Node* node = new Node(bid, key);
        ...
        //     found = found->next;
        //     while (found->next != nullptr) found = found->next;
        //     // Assign it to the last position
        //     found->next = node;
        // }
    }
}
